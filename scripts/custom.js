...
$(document).ready(function(){      
    ...
    //// LOGIN
        ...
        ////ADD ITEM / vacation
        const addItemForm3 = document.querySelector('#addItemForm3')

        addItemForm3.addEventListener('submit', (e) => {
            e.preventDefault();
            db.collection('mainCollection').add({
                userCustomID: userId, //userID variable comes from above on pageload
                itemName: addItemForm3.itemNameField3.value,
                checkStatus: 'unchecked',
                list: 'vacation',
                //try
                shareID: shareID
            ...
            //reset listItem input field and blur field
            addItemForm3.itemNameField3.value = '';
            addItemForm3.itemNameField3.blur();
        });

        ////ADD ITEM / customlist
        const addItemForm4 = document.querySelector('#addItemForm4')

        addItemForm4.addEventListener('submit', (e) => {
            e.preventDefault();
            db.collection('mainCollection').add({
                userCustomID: userId, //userID variable comes from above on pageload
                itemName: addItemForm4.itemNameField4.value,
                checkStatus: 'unchecked',
                list: 'custom',
                //try
                shareID: shareID
            ...
            //reset listItem input field and blur field
            addItemForm4.itemNameField4.value = '';
            addItemForm4.itemNameField4.blur();
        });
...
////RENDER LIST HTML FUNCTION
        ...
        function renderListItem3(doc) {            
            
            const daytriplist = document.querySelector('#vacationlist');
            let DIV = document.createElement('div');
            DIV.setAttribute('id', doc.id);
            vacationlist.appendChild(DIV);
            document.getElementById('guide3').classList.add('invisible');
            document.getElementById(doc.id).innerHTML = 
            '<span></span>'
            +'<input id="' + doc.id + 'box" type="checkbox" value="0" onclick="if(this.checked){this.parentElement.classList.add(\'checkedanimation\');}else{this.parentElement.classList.remove(\'checkedanimation\', \'linethrough\')}">'
            +'<label for="' + doc.id + 'box">' + doc.data().itemName + '</label>'
            +'<button id="' + doc.id + 'delete-button" class="icon-button fa font-16 fa-trash float-right color-nicebrown-light"></button>'   
            +'<div class="dividersolid"></div>';
            document.getElementById(doc.id).classList.add('fac', 'fac-checkbox-round', 'fac-nicebrown', 'list-item-margin')
        ...
        function renderListItem4(doc) {            
            
            const daytriplist = document.querySelector('#customlist');
            let DIV = document.createElement('div');
            DIV.setAttribute('id', doc.id);
            customlist.appendChild(DIV);
            document.getElementById('guide4').classList.add('invisible');
            document.getElementById(doc.id).innerHTML = 
            '<span></span>'
            +'<input id="' + doc.id + 'box" type="checkbox" value="0" onclick="if(this.checked){this.parentElement.classList.add(\'checkedanimation\');}else{this.parentElement.classList.remove(\'checkedanimation\', \'linethrough\')}">'
            +'<label for="' + doc.id + 'box">' + doc.data().itemName + '</label>'
                ...
                .onSnapshot(snapshot => {
                    ...
                    changes.forEach(change => {
                       ...
                       if(change.type == 'added' && change.doc.data().list == 'playground' && change.doc.data().shareID == shareID){
                        ...
                        else if(change.type == 'added' && change.doc.data().list == 'daytrip' && change.doc.data().shareID == shareID){
                            renderListItem2(change.doc);
                        }
                        else if(change.type == 'added' && change.doc.data().list == 'vacation' && change.doc.data().shareID == shareID){
                            renderListItem3(change.doc);
                        }
                        else if(change.type == 'added' && change.doc.data().list == 'custom' && change.doc.data().shareID == shareID){
...
//end try
        ...
        var colorsArray = [
            ...
            ["nicebeigedark","#99876A","#99876A","#99876A"],
